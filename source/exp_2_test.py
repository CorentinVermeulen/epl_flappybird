    ...
    for i, row in tqdm(df.iterrows()):
        ...
        for dur in durations:
            row = {'id': row['id'],
                   ...
                   'var': row['var'],
                   'duration': dur,
                   'duration_type': 'Baseline',
                   }
            dfi = pd.concat([dfi, pd.DataFrame(row, index=[0])])
    # Second test: moderate environment
    for i, row in tqdm(df.iterrows()):
        game_context = {'PLAYER_FLAP_ACC': -5,
                        'PLAYER_ACC_Y': 1,
                        'pipes_are_random':True,
                        'PLAYER_FLAP_ACC_VARIANCE': 1,
                        }
        ...
        for dur in durations:
            row = {'id': row['id'],
                   ...
                   'var': row['var'],
                   'duration': dur,
                   'duration_type': 'Moderate Random',
                   }
            dfi = pd.concat([dfi, pd.DataFrame(row, index=[0])])

    # third test: moderate environment
    for i, row in tqdm(df.iterrows()):
        game_context = {'PLAYER_FLAP_ACC': -5,
                        'PLAYER_ACC_Y': 1,
                        'pipes_are_random': True,
                        'PLAYER_FLAP_ACC_VARIANCE': 2.5,
                        }
...
def make_plots(dfi, xlabel="Jump Force Variance"):
    ...
    for dur in durations_types:
        ...
        plt.xlabel(xlabel)
        plt.ylabel('Duration')
        plt.savefig(f"{out_root}/EXP2TEST_boxplot_{dur}.pdf", format = 'pdf', dpi=300, bbox_inches='tight')
