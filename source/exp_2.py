...
update_rate: 0.99 or 0.999 ? 
UPDATE_TARGETNET_RATE: 1 3 or 5? 
"""
...
for j in range(len(update_rate)):
    ...
    for rep in range(iters):
        ...
        scores, durations = agent.train(show_progress=False, name=f'ur_{str(update_rate[j])}_{rep}')
        HS = np.max(scores)
        MD = np.mean(durations)
        MD_last = np.mean(durations[-250:])
        te = time.perf_counter() - t
        print(
            f"{j+1} {rep+1} - UR:{update_rate[j]}\n"
